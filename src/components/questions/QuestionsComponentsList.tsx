...
interface QuestionsComponentsListProps {
...
export function QuestionsComponentsList({ components, loading }: QuestionsComponentsListProps) {
  if (loading) {
    return (
  ...
  return (
    <div className="space-y-3">
      {components.map((component) => {
        ...
        const href = `/admin/questoes/componente/${componentToSlug(component.key)}`
              ...
              <div className="flex flex-wrap items-center gap-x-8 gap-y-4 xl:flex-1 xl:justify-center">
                ...
                <MetricCell label="Média">
                  <p className={cn('text-lg font-semibold tabular-nums', theme.color)}>
                    {component.avgPoints > 0 ? formatPointsLabel(component.avgPoints) : '—'}
                  </p>
                </MetricCell>
                <MetricCell label="Dificuldade">
                  <span
                    className={cn(
                      'inline-flex rounded-full px-3 py-1 text-xs font-medium',
                      component.dominantDifficulty ? theme.badge : 'bg-white/10 text-slate-500',
                    )}
                  >
                    {component.dominantDifficulty ?? '—'}
                  </span>
                </MetricCell>
