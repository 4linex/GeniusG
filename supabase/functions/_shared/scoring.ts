/**
 * Motor TRI/TCT — espelho de src/lib/scoring.ts para Edge Functions (Deno)
 */

import { clampTheta } from './scoreBounds.ts'
...
export function defaultTriParams(numAlternatives = 4): ItemTriParams {
  return { a: 1.0, b: 0, c: Math.round((1 / numAlternatives) * 1000) / 1000 }
}
...
export function processAssessment(
  ...
  return { percentualAcerto, theta, proficienciaEscala, nivelProficiencia, correct, total }
}
